...
class ReferenceChannel(Base):
    ...
    reference_channel_id: Mapped[str] = mapped_column(String, primary_key=True)
    youtube_channel_id: Mapped[str] = mapped_column(String, unique=True, index=True)
    schema_version: Mapped[str] = mapped_column(String, default="1.0.0")
    content_hash: Mapped[str] = mapped_column(String)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow, onupdate=utcnow)
    status: Mapped[str] = mapped_column(String, default="candidate", index=True)
    source: Mapped[str] = mapped_column(String, default="competitive_scout")
    ...
    confidence: Mapped[str] = mapped_column(String, default="baixa")
    last_seen_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    last_scouted_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
...
class ReferenceShort(Base):
    __tablename__ = "reference_shorts"

    reference_short_id: Mapped[str] = mapped_column(String, primary_key=True)
    youtube_video_id: Mapped[str] = mapped_column(String, unique=True, index=True)
    reference_channel_id: Mapped[str | None] = mapped_column(ForeignKey("reference_channels.reference_channel_id"), nullable=True, index=True)
    youtube_channel_id: Mapped[str] = mapped_column(String, index=True)
    schema_version: Mapped[str] = mapped_column(String, default="1.0.0")
    content_hash: Mapped[str] = mapped_column(String)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow, onupdate=utcnow)
    status: Mapped[str] = mapped_column(String, default="candidate", index=True)
    source_type: Mapped[str] = mapped_column(String, default="external")
    niche_id: Mapped[str] = mapped_column(String, default="curiosidades", index=True)
    line_id: Mapped[str | None] = mapped_column(String, nullable=True, index=True)
    title: Mapped[str] = mapped_column(String)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    confidence: Mapped[str] = mapped_column(String, default="baixa")
    transcript_source: Mapped[str] = mapped_column(String, default="none")
    analysis_artifact_path: Mapped[str | None] = mapped_column(String, nullable=True)
    performance_proxy: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class ScoutRun(Base):
    ...
    niche_id: Mapped[str] = mapped_column(String, default="curiosidades", index=True)
    source: Mapped[str] = mapped_column(String, default="competitive_scout")
    channels_considered: Mapped[int] = mapped_column(Integer, default=0)
    shorts_considered: Mapped[int] = mapped_column(Integer, default=0)
    shorts_selected: Mapped[int] = mapped_column(Integer, default=0)
    profiles_created: Mapped[int] = mapped_column(Integer, default=0)
...
class LearnedRetentionProfile(Base):
    ...
    title: Mapped[str] = mapped_column(String)
    confidence: Mapped[str] = mapped_column(String, default="baixa")
    source_run_id: Mapped[str | None] = mapped_column(ForeignKey("scout_runs.run_id"), nullable=True, index=True)
    approved_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    promoted_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    archived_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    analysis_artifact_path: Mapped[str | None] = mapped_column(String, nullable=True)
    supporting_reference_short_ids: Mapped[list] = mapped_column(JSON, default=list)
    dominant_skeleton: Mapped[dict] = mapped_column(JSON)
    metrics: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class RetentionExperiment(Base):
    ...
    profile_id: Mapped[str] = mapped_column(ForeignKey("learned_retention_profiles.profile_id"), index=True)
    schema_version: Mapped[str] = mapped_column(String, default="1.0.0")
    content_hash: Mapped[str] = mapped_column(String)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow, onupdate=utcnow)
    ...
    started_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    finished_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    target_job_count: Mapped[int] = mapped_column(Integer, default=3)
    decision: Mapped[str | None] = mapped_column(String, nullable=True)
    result_summary: Mapped[dict | None] = mapped_column(JSON, nullable=True)


class RetentionExperimentJob(Base):
    __tablename__ = "retention_experiment_jobs"
    __table_args__ = (UniqueConstraint("experiment_id", "job_id", name="uq_retention_experiment_job"),)

    experiment_job_id: Mapped[str] = mapped_column(String, primary_key=True)
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utcnow)
    status: Mapped[str] = mapped_column(String, default="planned", index=True)
    role: Mapped[str] = mapped_column(String, default="experiment")
    metrics: Mapped[dict | None] = mapped_column(JSON, nullable=True)
