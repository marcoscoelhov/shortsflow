...
class ScriptRepairDomain(BasePipeline):
    ...
    def _loop_closure_sentence(self, anchor: str, hook: str, payoff_hint: str, variant: int) -> str:
        ...
        hint = " ".join(str(payoff_hint or anchor).strip().strip(".!?").split()) or anchor
        templates = [
            f"Na segunda olhada, a {opening_ref} já preparava esse detalhe: {hint}.",
            f"Agora o começo muda de sentido: {hint} era a pista.",
            f"Repara no início outra vez: {anchor} já deixava esse sinal escondido.",
            f"O detalhe final devolve você ao começo, porque {hint} estava ali.",
            f"Volta para a primeira imagem e o truque aparece: {hint}.",
