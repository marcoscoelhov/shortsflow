...
The trade-off is that some compatibility imports and wrappers remain temporarily in `app/orchestrator.py`. Removing them is an incremental cleanup task, not a reason to move domain rules back into the shell. Removed implementations are recovered from Git history instead of being kept in an in-repository quarantine.
